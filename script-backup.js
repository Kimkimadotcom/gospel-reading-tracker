...
/* ===================================================== SERVICE WORKER ===================================================== */ if ("serviceWorker" in navigator) { window.addEventListener("load", async () => { try { const registration = await navigator.serviceWorker.register("sw.js"); console.log("Service Worker registered:", registration.scope); /* Ask the browser to check for a new service worker. */ await registration.update(); /* If a new service worker is waiting, activate it immediately. */ if (registration.waiting) { registration.waiting.postMessage({ type: "SKIP_WAITING" }); } } catch (error) { console.error("Service Worker registration failed:", error); } }); /* When the new service worker takes control, reload the page once. This allows the newest files and images to appear automatically. */ let refreshing = false; navigator.serviceWorker.addEventListener("controllerchange", () => { if (refreshing) { return; } refreshing = true; window.location.reload(); }); }
...
/* =====================================================
   STEP 34 — FAVORITE PASSAGES DATA
===================================================== */

let favoritePassages =
    JSON.parse(localStorage.getItem("favoritePassages")) || [];


/* Save favorite passages */

function saveFavoritePassages() {

    localStorage.setItem(
        "favoritePassages",
        JSON.stringify(favoritePassages)
    );
